...
void Run6DSensorExample(OptoDAQ & p_optoDAQ, mytime_t p_tTimeout, ros::NodeHandle nh)
{

	ros::Publisher optoPub=nh.advertise<geometry_msgs::WrenchStamped>("optoforce", 1000);
	while(ros::ok()){
		...
		msg.wrench.force.x=optoPackage.Fx;
		msg.wrench.force.y=optoPackage.Fy;
		msg.wrench.force.z=optoPackage.Fz;
		msg.wrench.torque.x=optoPackage.Tx;
		msg.wrench.torque.y=optoPackage.Ty;
		msg.wrench.torque.z=optoPackage.Tz;
		optoPub.publish(msg);
		ros::spinOnce();
...
int main(int argc, char** argv)
{
	...
	if (Is3DSensor(optoDaq)) {
		...
		Run6DSensorExample(optoDaq, tRunningTime, nh);
	}
	optoDaq.close();
    return 0;
}

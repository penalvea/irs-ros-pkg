  ...
  if (drawing_enabled)
  {
    for (int i = 0; i < rec_.size(); i++)
    {
      ...
      cv::line(image, cv::Point(0, rec_[i]->getPeakDetector()->getLimits()[0]),
               cv::Point(Ic.getCols() - 1, rec_[i]->getPeakDetector()->getLimits()[0]), cv::Scalar(255, 0, 0));
      cv::line(image, cv::Point(0, rec_[i]->getPeakDetector()->getLimits()[1]),
               cv::Point(Ic.getCols() - 1, rec_[i]->getPeakDetector()->getLimits()[1]), cv::Scalar(255, 0, 0));
      for (unsigned int j = 0; j < rec_[i]->getPeakDetector()->points.size(); j++)
      {
        ...
        cv::Vec3b & pixel = image.at<cv::Vec3b>(rec_[i]->getPeakDetector()->points[j][0],
                                                rec_[i]->getPeakDetector()->points[j][1]);
        pixel[0] = 0;
        pixel[1] = 0;
        pixel[2] = 255;

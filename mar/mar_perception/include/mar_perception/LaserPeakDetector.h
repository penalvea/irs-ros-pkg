...
public:
	...
	void setRadiusMin(double radius){min_radius_=radius;}
	void setRadiusMax(double radius){max_radius_=radius;}
	void setLimits(vpHomogeneousMatrix bMc, vpHomogeneousMatrix bMl);
	virtual void perceive()=0;

	std::vector<int> getLimits(){ return limits_;}
	std::vector<vpColVector> &getPeaks() {return points;}
	VirtualImagePtr getGrabber() {return grabber_;}
...
public:
	LastImageSubPixelLaserPeakDetector(VirtualImagePtr grabber):LaserPeakDetector(grabber) {

...
double LaserPeakDetector::getIntersectionPoint(vpHomogeneousMatrix bMc, vpHomogeneousMatrix bMl, int col, double radius)
{
  ...
  double a, b, c;
  a = std::pow((intersection_point2[0] - intersection_point1[0]), 2.0)
      + std::pow((intersection_point2[1] - intersection_point1[1]), 2.0)
      + std::pow((intersection_point2[2] - intersection_point1[2]), 2.0);
  b = 2
      * (((intersection_point2[0] - intersection_point1[0]) * (intersection_point1[0] - sphere_centre[0]))
          + ((intersection_point2[1] - intersection_point1[1]) * (intersection_point1[1] - sphere_centre[1]))
          + ((intersection_point2[2] - intersection_point1[2]) * (intersection_point1[2] - sphere_centre[2])));
  c = std::pow((intersection_point1[0] - sphere_centre[0]), 2.0)
      + std::pow((intersection_point1[1] - sphere_centre[1]), 2.0)
      + std::pow((intersection_point1[2] - sphere_centre[2]), 2.0) - std::pow(radius, 2.0);
...
void SimpleLaserPeakDetector::perceive()
{
  ...
  cvCreateTrackbar("Red", "Control", &r_, 255);
  cvCreateTrackbar("Green", "Control", &g_, 255);
  cvCreateTrackbar("Blue", "Control", &b_, 255);
  cvCreateTrackbar("tolerance_", "Control", &tolerance_, 300);
  reference_color_.R=r_;
  reference_color_.G=g_;
  reference_color_.B=b_;
  ...
  //For each two columns, search the peak according to the RGB color (most similar to reference_color_)
    ...
    for (unsigned int c = 0; c < grabber_->image.getCols(); c++)
    {
      ...
      for (unsigned int r = 0; r < grabber_->image.getRows(); r++)
      {
        ...
        {
          if(grabber_->image(r, c).G>green){
            similarity=localsim;
            mr = r;
            mc = c;
            green=grabber_->image(r, c).G;
          }
        }
      }
      if (similarity < tolerance_)
  ...
  for (unsigned int c = 0; c < grabber_->image.getCols(); c++)
  {
    ...
    for (int r = limits_[0]; r <= limits_[1]; r++)
    {
      ...
      if (localsim < tolerance_)
      {
        ...
        mc += c;
        nmatch++;
        vpColVector point2(2);
        point2[0] = r;
        point2[1] = c;
        points.push_back(point2);

        if(grabber_->image(r, c).R+ grabber_->image(r, c).G+ grabber_->image(r, c).B>max){
          max=grabber_->image(r, c).R+ grabber_->image(r, c).G+ grabber_->image(r, c).B;
          maxr=r;
          maxc=c;
        }
      }
...
HSVSubPixelLaserPeakDetector::HSVSubPixelLaserPeakDetector(VirtualImagePtr grabber):
...
SimulationLaserPeakDetector::SimulationLaserPeakDetector(VirtualImagePtr grabber) :
    LaserPeakDetector(grabber)
{
